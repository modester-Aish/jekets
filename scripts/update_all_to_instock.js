...
function updateStockStatus(productId, stockStatus = 'instock') {
  return new Promise((resolve, reject) => {
    const auth = Buffer.from(`${CONSUMER_KEY}:${CONSUMER_SECRET}`).toString('base64');
...
// Main function
async function updateAllToInStock() {
  ...
  try {
    ...
    const outOfStockProducts = products.filter(p => p.stock_status !== 'instock');
    ...
    console.log('🔄 Updating stock status to "in stock"...\n');
    ...
    for (const product of outOfStockProducts) {
      try {
        ...
        // Delay to avoid rate limiting (1 second between updates)
...
if (require.main === module) {
  updateAllToInStock().catch(console.error);
}

module.exports = { updateAllToInStock };
